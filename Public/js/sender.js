...
var bathUrl = currentFilePath().replace(/sender.js(.)*/, "");

ws = {};
if (typeof console == "undefined") {    this.console = { log: function (msg) {  } };}

WEB_SOCKET_SWF_LOCATION = bathUrl+"swf/WebSocketMain.swf";
...
window.onload = function()
{
	...
	ws.onopen = function() {
		// 发送数据
		$.post("getUid",{}, function(data){
			if(data.id !=0){
				ws.send(JSON.stringify({"type":"group","id":data.id}));
			}
		},"json");		
	...
	ws.onmessage = function(e) {
	 ...
	 // console.log(data['type']);
	  ...
	  switch(data['type']){
	  	...
	  	case 'remove': 	
	  		//alert(data['type']);
	  		if($('#neworder').length>0 && typeof(refresh_order)=="function"){
	  				refresh_order(data['orderid']);  				  			
	  		}
	  		break;
	  	case 'grab':
	  		//alert(data['type']);
	  		if($('#neworder').length>0 && typeof(refresh_grab)=="function"){
	  			refresh_grab(data['orderid']);  				  			
	  		}
	  		break;	  		
